...
const fetchModelsData = () => {
    ...
    return http.get(modelsUrl).then(response => {
        return ({
          data: response.data.data.map(item => ({ ...item.attributes, id: item.id })),
        })
    }).catch(error => error)
}
...
exports.sourceNodes = async ({ actions, createNodeId, createContentDigest }) => {
  let modelNodeArray = []
     await fetchModelsData().then(async ({ data: models }) => {
      ...
      await fetchProducts().then(async ({ data: products }) => {
        await products.forEach(product => {
          const productNodeId = createNodeId(`${product.ref}-${product.name}`)
          const findParentModelNode = modelNodeArray.find(model => model.modelId * 1 === product.model_id)
          const productNode = {
            name: product.name,
